...
for(let i=0; i<optionLen; i++){
  ...
  // timeout for every question
//   const timeOut = setTimeout(next, 25000);
// console.log(timeOut);
  questionCounter++;
}

//get the result of current attmpt question
...
// if the answer is incorrect then show the correct option by adding green color to the correct option
   ...
   for(let i = 0; i<optionLen; i++){
    ...
    // setted timeout to pass to the next question
    const timeOut = setTimeout(next, 1000);
...
function goToHome(){
    ...
    resultBox.classList.add("hide");
    // show home box
    shareWithFriend.classList.remove("hide");
    resetQuiz();
}
